...
console.log(person2.firstName); // jhon
...
console.log(person.address == personTwo.address); // true
console.log(personTwo.address.city); // San Joses
console.log(person.address.city); // San Joses
...
let allBlogesClone = {
  ...bloges,
  author: { ...bloges.author },
  comments: [...bloges.comments],
};
```
...
let clonedPerson = [...person, input:{...person.input}, output:{...person.output},input.name{...person.input.name},input.birthday:{...person.input.birthaday}, output:{...person.output} ]
```
...
function cloneObject(user) {
  return { ...user, sister: { ...user.sister } };
}
